...
class TestRegularizedModel:
    def test_ridge(self, synthetic_data: tuple[np.ndarray, FactorData]) -> None:
        ...
        result = compute_regularized_model(returns, fd, method="ridge", n_bootstrap=20)
    ...
    def test_bootstrap_ses(self, synthetic_data: tuple[np.ndarray, FactorData]) -> None:
        returns, fd = synthetic_data
        result = compute_regularized_model(returns, fd, method="ridge", n_bootstrap=50)
        for f in fd.factor_names:
            assert result.beta_ses[f] > 0
            assert result.beta_cis[f][0] < result.beta_cis[f][1]

    def test_beta_recovery_ridge(self, synthetic_data: tuple[np.ndarray, FactorData]) -> None:
        returns, fd = synthetic_data
        result = compute_regularized_model(returns, fd, method="ridge", alpha=0.001, n_bootstrap=20)
